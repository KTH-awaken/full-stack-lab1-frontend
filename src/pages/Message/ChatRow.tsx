...
const ChatRow = ({ chat }: { chat: ChatApi }) => {
    ...
    return (
        <NavLink className={({ isActive }) => isActive ? `bg-accent rounded-xl border-none ${classes}` : classes} to={`/messages/${chat.otherParticipantId.substring(0, chat.otherParticipantId.length - 4)}`}>
            <Avatar>
